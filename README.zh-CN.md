...
下面的命令参考主要面向维护者、集成者、CI 和高级排障场景。日常使用不需要开发者手动执行 DroidLens 命令。
...
可选：如果需要检查 DroidLens 是否已正确接入项目，可以在项目根目录做一次手动冒烟验证：
...
在 Claude Code / Codex 中使用时，AI 应该自动完成初始化。下面的命令只用于安装后的可选验证或排障：
...
skill 会告诉 AI 使用 `scripts/droidlens/droidlens`，自行完成环境检查，优先使用 XML/summary，避免写死坐标，并只在必要时请求有限范围的危险操作授权。
